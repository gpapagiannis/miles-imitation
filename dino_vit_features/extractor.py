"""
Originally authored by Shir Amir: https://github.com/ShirAmir/dino-vit-features
The code has been modified for this work.
"""
...
class ViTExtractor:
    ...
    def create_model(model_type: str) -> nn.Module:
        ...
        if 'dino' in model_type:
            # print("hey", model_type)
            if model_type == 'dino_vitb8':
                print("loading dino_vitb8")

                model = torch.hub.load('facebookresearch/dino:main', model_type, source='github')
            else:
                model = torch.hub.load('/home/george/catkin_ws/facebookresearch/dinov2', 'dinov2_vitb14', source='local')
                print("loaded dino_vitb14")
    ...
    def preprocess(self, image_path: Union[str, Path],
                   load_size: Union[int, Tuple[int, int]] = None, adjust_contrast=False) -> Tuple[torch.Tensor, Image.Image]:
        """
        ...
        if load_size is not None:
            ...
            if adjust_contrast:
                from torchvision.transforms.functional import adjust_brightness, \
                    adjust_contrast
                import random
                pil_image  = adjust_contrast(pil_image,  random.uniform(0.2, 1.5))
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[torch.Tensor]:
        ...
        self.load_size = (H, W)
        # print(self.p, self.stride)
        self.p = self.p[0] if isinstance(self.p, tuple) else self.p
